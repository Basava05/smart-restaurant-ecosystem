...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      else navigate('/'); // default customer
    } catch (err) {
      const data = err.response?.data;
      if (data?.errors && data.errors.length > 0) {
        addToast(data.errors[0].message, 'error');
      } else {
        addToast(data?.error || err.message || 'Login failed', 'error');
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 bg-gradient-to-br from-slate-50 to-neutral-100 relative overflow-hidden">
      ...
      <div className="absolute top-[-100px] right-[-100px] w-[400px] h-[400px] rounded-full bg-ember/5 blur-3xl" />
      <div className="absolute bottom-[-100px] left-[-100px] w-[350px] h-[350px] rounded-full bg-herb/5 blur-3xl" />
          ...
          <div className="p-8">
            <form onSubmit={handleLogin} className="space-y-5">
              ...
              <motion.button
                ...
                disabled={loading}
                whileTap={{ scale: 0.97 }}
                className="w-full bg-ember hover:bg-ember/90 text-white font-semibold py-3.5 rounded-xl text-base transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-md"
              >
                {loading ? (
